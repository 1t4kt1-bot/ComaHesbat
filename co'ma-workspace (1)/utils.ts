...
import { Order, PricingConfig, SessionEvent, SessionSegment, DeviceStatus, LoanInstallment } from './types';
...
export const generateLoanInstallments = (
    ...
    const amountPerInstallment = Math.ceil(principal / count); // Ceil to ensure coverage
    
    // Adjust last installment to match exact principal
    let runningTotal = 0;
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (i === count - 1) {
            amount = principal - runningTotal; // Fix rounding diff on last one
        }
        runningTotal += amount;
