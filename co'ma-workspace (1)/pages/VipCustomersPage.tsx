...
const VipCustomersPage: React.FC<VipCustomersPageProps> = ({ customers, onUpdateCustomers }) => {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'vip' | 'debt' | 'credit'>('all');
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
          <div className="bg-white p-1 rounded-xl border border-gray-200 shadow-sm flex w-full md:w-auto">
              ...
              <button onClick={() => setActiveTab('vip')} className={`flex-1 md:flex-none px-4 py-2 rounded-lg text-sm font-bold transition-colors flex items-center justify-center gap-1 ${activeTab === 'vip' ? 'bg-yellow-50 text-yellow-700' : 'text-gray-500 hover:bg-gray-50'}`}>
                  <Crown size={14}/> VIP
              </button>
