...
import { Wallet, Plus, Trash2, Calendar, Building2, User, Info, ArrowDownLeft, Banknote, Landmark } from 'lucide-react';
...
const PartnerDebtsPage: React.FC<PartnerDebtsPageProps> = ({ debtsList, onUpdateDebtsList, bankAccounts, onAddDebt }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in text-gray-900">
                        ...
                        <div className="flex-1 bg-white p-4 overflow-y-auto max-h-60">
                            {stats.items.length === 0 ? (
                                ...
                                <div className="space-y-3">
                                    {stats.items.map(d => (
                                        <div key={d.id} className={`flex justify-between items-start p-3 rounded-lg border transition-colors group ${d.amount < 0 ? 'bg-emerald-50 border-emerald-100' : (d.debtSource === 'partner' ? 'bg-gray-50 border-gray-100' : 'bg-white border-red-100 hover:bg-red-50')}`}>
                                            <div>
        ...
        <ConfirmModal isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={() => { if (deleteId) onUpdateDebtsList(debtsList.filter(d => d.id !== deleteId)); }} message="حذف؟" />
    </div>
  );
};
