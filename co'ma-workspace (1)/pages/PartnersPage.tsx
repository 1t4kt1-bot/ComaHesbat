...
const PartnersPage: React.FC<PartnersPageProps> = ({ ledger }) => {
  ...
  const activeLedger = activeStats ? activeStats.entries.filter(i => ledgerFilter === 'all' || i.channel === ledgerFilter).sort((a,b) => b.timestamp.localeCompare(a.timestamp)) : [];
                            ...
                            {activeLedger.length === 0 ? (
                                ...
                                activeLedger.map(item => (
                                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {item.channel === 'cash' ? (
                                                <span className="flex items-center gap-1 text-emerald-600 text-xs font-bold"><BanknoteIcon size={12}/> كاش</span>
                                            ) : (
                                                <span className="flex items-center gap-1 text-blue-600 text-xs font-bold"><LandmarkIcon size={12}/> بنك</span>
                                            )}
                                        </td>
